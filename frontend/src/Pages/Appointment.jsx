    ...
    return docInfo && (
        <div className='mt-32 ml-32 mr-32'>
            ...
            <div className='flex flex-col sm:flex-row gap-14'>
                ...
                <div className='flex-1 border border-gray-400 rounded-lg p-8 py-2 bg-white mx-2 sm:mx-0 mt-[-80px] sm:mt-0'>
                    ...
                    <div>
                        ...
                        <p className='twxt-sm text-gray-500 max-w-[700px] mt-1'>{docInfo.about}</p>
                    </div>
            ...
            <div className='sm:ml-80 sm:pl-4 mt-4 font-medium  text-gray-700'>
              ...
              <div className='flex items-center gap-3 w-full overflow-x-scroll mt-4'>
                {docSlots.length && docSlots[slotIndex].map((item,index)=>(
                    <p onClick={()=>setSlotTime(item.time)} className={`text-sm font-light flex-shrink-0 px-5 py-2 rounded-full cursor-pointer ${item.time === slotTime ? 'bg-green-400 text-white': 'text-gray-400 border border-gray-300' }`} key={index}>{item.time.toLowerCase()}</p>
                ))}
              </div>
